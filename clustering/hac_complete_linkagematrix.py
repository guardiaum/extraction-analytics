...
# finds the minimum similarities btw clusters on 
...
def mergeMatrix(clusters, matrix):
	...
	if(cluster1Index!=-1 and cluster2Index!=-1 and minSimilarity!=0.00):
		i = 0 
		while(i<len(matrix)):
			if(matrix[i]!=None):
				i = i + 1
			else:
				matrix[i] = [cluster1Index, cluster2Index, minSimilarity, count]
				break;
