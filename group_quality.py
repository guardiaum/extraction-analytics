...
for categoryName in categoriesName:
	...
	print("Infobox mapping, missing template: %s " % articlesWithInfobox[~np.isin(articlesWithInfobox[:, 0], articlesWithTemplate[:,0]), 0])
	print("Template mapping, missing infobox: %s " % articlesWithTemplate[~np.isin(articlesWithTemplate[:, 0], articlesWithInfobox[:, 0]), 0])
